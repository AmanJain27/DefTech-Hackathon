...
def diffprivacy(epsilons, dp, X_train, y_train, X_test, y_test, y_arr, y, x):
    ...
    for d in range(1,12,2):
        ...
        for e in epsilons:      
            print(f"FOR EPSILON = {e}")
            decision = dp.DecisionTreeClassifier(max_depth=d, e=e, s=1, min_samples_leaf=5)
            #decision = DecisionTreeClassifier(max_depth=d)
            decision.fit(X_train, y_train)
            print(type(decision.tree_.n_node_samples))
            print(decision.tree_.n_node_samples)
            print("n_outputs")
            print(decision.tree_.n_outputs)
            
            n_node_samples = list(decision.tree_.n_node_samples)
            n_node_samples =  [float(x) for x in n_node_samples]
            #print(n_node_samples)
            print("noise added")
            bounds = (min(n_node_samples), max(n_node_samples))
            noisy_samples = ln.Transformation(n_node_samples, 1/e)
            print(noisy_samples)
            print("node count")
            print(decision.tree_.node_count)
            print("children left")
            print(decision.tree_.children_left)
            print("children right")
            print(decision.tree_.children_right)
            a_n = decision.score(X_test, y_test)
            # print accuracy before noise and after noise
            
            print("before noise accuracy = ", a_n)
            # change the node samples of the decision tree to check the effect on accuracy
            #decision.tree_.n_node_samples = np.array(n_node_samples) 
            print("after noise accuracy = ", a_n)

            print("predicitions")
            print(len(decision.predict(X_test)))
